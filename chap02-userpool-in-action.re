...
//abstract{
UserPoolを利用したアーキテクチャと、開発・設定のポイントについてご紹介します。
//}

=={sec-ext} UserPoolを採用したシステムのアーキテクチャ
コンシューマー向けのWebサイトにおいて、Cognitoを採用したあアーキテクチャを2通り紹介します。  
ここではクライアント・サーバーともに同じサービス事業者が開発している前提で解説します。
サービスを別の事業者のクライアントに提供する場合、UserPoolによるOAuthの提供について調べてみてください。  
また、ユースケースとしては、他にもSPAにログイン機能を実装するケースや、エンタープライズの認証基盤の認証基盤として使うケースがあります。
...
個人的には、認証はマネージドサービス(API Gateway/ ALB)に任せ、エンジニアはサービス開発に集中するのが良いと思います。
認証周りを自前で実装しないでよいだけでなく、ローカルで認証レスなデバッグができるからです。
結合テストなども実装しやすいのではないでしょうか。

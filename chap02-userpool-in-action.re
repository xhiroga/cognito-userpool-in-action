...
=={sec-ext} UserPoolを利用したシステムのアーキテクチャ
コンシューマー向けのWebサイトにおいて、Cognitoを利用したシステムを2通り紹介します。  
...
=={sec-ext} UserPoolを利用したシステムの開発

=== 設定のポイント
UserPoolには多くの設定があります。
マネジメントコンソールではデフォルトの設定が用意されてはいますが、本番で運用する前に@<b>{絶対に把握すべき設定}・把握したほうが良い設定もあります。いくつか見ていきましょう。@<br>{}
...
//image[aliases][全てのエイリアス属性が選択された状態][scale=0.8]

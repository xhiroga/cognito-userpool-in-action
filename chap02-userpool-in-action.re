...
コンシューマー向けのWebサイトにおいて、Cognitoを採用したシステムを2通り紹介します。  
...
//image[architecture_1][UserPoolをユーザー認証・認可基盤として利用するケース][scale=0.8]
 ...
 - 3. クライアントは受け取ったクレデンシャルを用いてリソースサーバーにリクエストします。

上記フローの1〜2については、以下の3通りの実装パターンが考えられます。
@<br>{}

 - A. UserPoolがホストする組み込みのサインイン・サインアップページを利用する。
 - B. JavaScriptアプリ/ネイティブアプリで、AWS Amplify（またはAWS SDK）を利用してUserPoolのAPIを直接呼び出す。
 - C. サインアップ・サインインを提供するAPIを実装し、サーバーサイドでUserPoolのAPIを呼び出す。

クライアントとリソースサーバーの提供元が同じ場合、パターンB.を利用するのが最もシンプルでしょう。  
クライアントとリソースサーバーの提供元が異なる場合は、リソースサーバーへのアクセスをOAuth2.0に則って管理したいはずです。パターンA.を利用すれば画面を自前で実装しなくて良くなります。  
ただし、パターンA.で利用している組み込みのサインイン・サインアップページは、UserPoolのカスタム認証フロー（自前で構築したパスワードレス認証など）には対応していません。その場合は、サインイン・サインアップページも自前で実装することになります。  
@<br>{}

また、リソースサーバーへのアクセスについても以下の3通りのパターンが考えられます。
@<br>{}

 - 1. API Gatewayのオーソライザーを用いてリクエストを検証する。
 - 2. ALBの認証アクションを用いてリクエストを検証する。
 - 3. リクエストの検証処理を自前でサーバーに実装する。

クライアントとリソースサーバーの提供元が同じ場合は、どのパターンでも実装が可能です。リクエストにセットされたIDトークンをUserPoolに対して検証できれば問題ありません。  
クライアントとリソースサーバーの提供元が異なる場合は、IDトークンではなくアクセストークンを受け取るはずです。API Gatewayのカスタムオーソライザーなどで検証処理を実装しましょう。
@<br>{}

//note[コラム: アプリケーション内でリクエストの検証をしないと嬉しいこと]{
アプリケーション内でリクエストの検証をしないと、実装すべき機能が減る以外にも嬉しいことがあります。  
それは、ローカルで認証レスなデバッグができることです。  
...
エンドユーザーがAWSリソースに直接アクセスするケースでは、エンドユーザーに対する一時的なAWS認証情報（アクセスキー、シークレットキー）を発行し、適切に管理しなくてはなりません。
AWSではFederated Identityを利用します。エンドユーザーにAWSリソースにアクセスさせたいクライアントは、UserPoolのIDトークンをFederated Identityに渡すことで一時的なAWS認証情報を渡すことができます。
一時的なAWS認証情報は事前に定義したIAM Roleと対応しているため、IAMロールのポリシーやリソースポリシーを用いて権限を設定することが可能です。

サーバーによるユーザー認証は、API Gatewayの場合はIAM認証を用います。AWS認証情報を検証し、対応するUserPoolのSubをリソースサーバーに伝えてくれるため重宝します。
...
ユーザーの存在チェックをするケースでは、ユーザーがログインに用いた一意識別子（＝usernameもしくはエイリアス属性）を利用することが多いと考えられます。
...
このうち、ForgotPasswordでは古いパスワードを無効にすることはできませんが、AdminResetUserPasswordでは無効にすることができます。
これは、ForgotPasswordが認証なしで誰でもリクエスト可能なAPIだからであると考えられます。  
...
セキュリティの観点からも、アクセス解析などで活用する事を考えても、クライアントIDはクライアントごとに発行してください。
...
アカウントの有効期限とは、つまりアカウント作成時に設定された初期パスワードの有効期限です。
初期パスワードは管理者が指定できたり、メールやSMSで送信されたりするので、セキュリティを担保するために有効期限がついていると考えられます。
この有効期限は、特に指定しない場合はデフォルトで7日間となっています。
どのような値にするかは要件次第ですが、ユーザーから初期パスワードが無効になってログインできない問い合わせが来たときにすぐに原因が分かるよう、明示的に値を指定すべきだと考えます。


=== メールの送信元にAWS SESを設定する。
メールアドレスの検証や初期パスワードの送信などで、ユーザーにメールを送信することがあります。
UserPoolではデフォルトのメールの設定がありますが、以下の理由から利用するのは開発環境などに限定したほうがよいでしょう。@<fn>{993B91D2}  

...
=={sec-ext} ユーザー移行LambdaトリガーとCSVファイルのインポート、どちらを利用すべき？

ユーザー移行LambdaトリガーとCSVファイルのインポートのどちらを利用すべきでしょうか。まずは表（@<img>{user_migration_pros_cons}）をご覧ください。  
//image[user_migration_pros_cons][ユーザー移行Lambdaトリガーの処理フロー][scale=0.8]
@<br>{}

最大のポイントはパスワードの移行の可否です。ユーザーにパスワードの変更を案内できるのであれば、CSVファイルによるユーザーのインポートで問題ないでしょう。  
シームレスなユーザー移行が必要な場合はユーザー移行Lambdaトリガーを利用することになります。しかし、この方法だといつまでもUserPoolに移行されないユーザーがいるかもしれません。  
一定期間はユーザー移行Lambdaトリガーを運用し、大多数のユーザーが移行できたところでCSVファイルのインポートに切り替える、という方法も考えられるでしょう。

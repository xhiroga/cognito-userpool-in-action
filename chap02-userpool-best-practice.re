...
なお、メールアドレスや電話番号をユーザー名のエイリアスとして使うのではなく、ユーザー名そのものとして利用することも可能です。その場合、アプリのインストール時に初めからアカウントが設定されているようなユーザー体験を作ることが難しくなります。
...
あくまで私見ですが、次に紹介するケースではクライアントアプリによる自己サインアップを許可しないほうがよいと考えます。
 ...
 1. メールアドレスでサインアップする際に、ケースインセンシティブ（メールアドレスの大文字・小文字を問わず重複を許可しない）ようにしたい
 2. UserPool以外のユーザー管理サービスを併用しており、そのサービスで既にユーザーがいないかをサインアップ時に確認したい
...
ただし、UserPoolとFederated Identityを接続して利用する場合は、Federated IdentityのIdentity IDを利用したほうがよいでしょう。理由は、Facebookアカウントなどを使ってFedetared Identity経由で直接サインインされた場合のIDが必要となるためです。

...
AWSが提供するユーザーの認証・認可を管理するためのマネージドサービスです。GCPであればFirebase Authenticationが競合にあたります。  
...
まとめると、AWSの一時的な認証情報を発行したい場合はFederated Identityを利用し、その他の場合はUserPoolを利用すればよいことが分かりました。  
次章からは、実際のアーキテクチャ、開発と設定のポイント、ユーザー移行のポイントなどを、UserPoolに焦点を当ててご紹介します。
